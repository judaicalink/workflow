...
def generate_ep_graph_sep(index, save=False, return_graph=False, outpath="", rdf_format=''):
    ...
    for ep_uri, aliases in index.items():
        for alias in aliases:
            ...
            o = URIRef(alias)
            ep_graph.add((s, OWL.sameAs, o))
            if alias.startswith('http://data.judaicalink.org/'):
                ep_graph.add((s, jl.isEntityPageOf, o))
